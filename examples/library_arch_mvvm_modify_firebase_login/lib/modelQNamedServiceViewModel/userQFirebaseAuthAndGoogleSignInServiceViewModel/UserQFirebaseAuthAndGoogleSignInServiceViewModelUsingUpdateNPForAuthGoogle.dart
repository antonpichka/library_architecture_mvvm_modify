...
base class UserQFirebaseAuthAndGoogleSignInServiceViewModelUsingUpdateNPForAuthGoogle<
        T extends User,
  ...
  @protected
  @override
  Future<Result<bool>?> updateModelToNamedServiceNPDS() async {
    try {
      late final firebase_auth.AuthCredential credential;
      if (kIsWeb) {
        final googleProvider = firebase_auth.GoogleAuthProvider();
        final userCredential = await firebaseAuthService.getFirebaseAuth
            ?.signInWithPopup(googleProvider);
        credential = userCredential!.credential!;
      } else {
        final googleUser = await googleSignInService.getGoogleSignIn?.signIn();
