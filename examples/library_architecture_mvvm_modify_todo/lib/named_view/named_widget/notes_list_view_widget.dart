    ...
    extends StatelessWidget
{
  ...
  Widget buildSuccess(BuildContext context,NotesIsLoading? notesIsLoading) {
    return CupertinoScrollbar(
      child: ListView(
        children: [
          for(final note in notesIsLoading?.listNote?.listModel ?? List.empty())
  ...
  @protected
  Widget buildItemForSuccess(BuildContext context,NotesIsLoading? notesIsLoading,Note? note) {
    final theme = Theme.of(context);
    final captionColor = theme.textTheme.caption?.color;
    return Dismissible(
      key: Key('noteListTile_dismissible_${note?.uuId}'),
      onDismissed: (_) {
        lo.deleteNoteToSqfliteServiceParameterStringAndInGeneralOneTask(note, (lastDeletedNote) {
          scaffoldMessengerStateForDeletedNote(context,lastDeletedNote,(insertNote) {
            lo.insertNoteToSqfliteServiceParameterStringAndInGeneralOneTask(insertNote);
          });
        });
      },
