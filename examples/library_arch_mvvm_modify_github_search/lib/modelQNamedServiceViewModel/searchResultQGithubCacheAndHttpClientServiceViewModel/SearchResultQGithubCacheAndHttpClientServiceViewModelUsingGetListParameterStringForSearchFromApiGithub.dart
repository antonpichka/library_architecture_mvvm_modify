...
base class SearchResultQGithubCacheAndHttpClientServiceViewModelUsingGetListParameterStringForSearchFromApiGithub<
  ...
  Future<Y?> getListModelFromNamedServiceParameterNamedDS(
    ...
    try {
      final object = githubCacheService.getGithubCache?.get(parameter);
      final listSearchResultFromObjectASameGithubCache =
          getListSearchResultFromObject(object);
      if (listSearchResultFromObjectASameGithubCache != null) {
        return listSearchResultFromObjectASameGithubCache;
      }
      final response = await httpClientService.getHttpClient?.get(
          Uri.parse('https://api.github.com/search/repositories?q=$parameter'));
      if (response!.statusCode != 200) {
        throw NetworkException.fromStatusCode(this, response.statusCode);
      }
      final mapFromJsonDecode =
          json.decode(response.body) as Map<String, dynamic>;
      final listSearchResultFromMapASameHttpClient =
          getListSearchResultFromMap(mapFromJsonDecode);
      githubCacheService.getGithubCache
      ...
      return getListSearchResultFromBaseException(LocalException(
  ...
  Y? getListSearchResultFromMap(Map<String, dynamic>? map) {
    final listSearchResult =
        (map![ListSearchResult.constParameterListModel] as List<dynamic>)
            .map((dynamic item) {
      ...
      return SearchResult.success(
          ...
          GithubUser.success(
              mapSearchResult[SearchResult.constParameterGithubUser]
                  [GithubUser.constParameterLogin] as String,
              mapSearchResult[SearchResult.constParameterGithubUser]
                  [GithubUser.constParameterAvatarUrl] as String));
    }).toList();
